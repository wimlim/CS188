...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            successor_state = ((nextx, nexty), tuple(visited))
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    if len(unvisited) == 0:
        return 0
    max_dist = max([util.manhattanDistance((x, y), corner) for corner in unvisited])
    return max_dist
    ...
    def findPathToClosestDot(self, gameState: pacman.GameState):
        ...
        while not queue.isEmpty():
            ...
            if food[position[0]][position[1]]:
                return actions
            for next_position, action, _ in problem.getSuccessors((position)):
                queue.push((next_position, actions + [action]))
